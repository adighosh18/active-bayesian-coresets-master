...
class NeuralLinear(torch.nn.Module):
    ...
    def test(self, data, save_dir_path, dataset_name, start_time, **kwargs):
        """
        Test model
        ...
        if not Path(save_dir_path).is_dir():
            utils.create_dir(save_dir_path)

        file_path = os.path.join(save_dir_path, dataset_name + ".csv")
        ...
        with open(file_path, 'a') as data_csv:
            datawriter = writer(data_csv)
            datawriter.writerow({len(data.index['train']), np.mean(performances), time.time() - self.timenow})
            self.timenow = time.time()
            data_csv.close()
